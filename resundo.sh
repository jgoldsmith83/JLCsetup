...
function remove_vnc_only() {
	sudo apt-get -y remove --purge x11vnc
...
function remove_vnc_ssh() {
	sudo apt-get -y remove --purge openssh && sudo apt-get remove --purge opensshd
	sudo apt-get -y remove --purge x11vnc
	sudo rm /etc/x11vnc.pass
	sudo rm /etc/init/x11vnc.conf
	sudo rm /usr/shared/applications/x11vnc.desktop
...
case $remote in
1)
	sudo apt-get -y remove --purge openssh && sudo apt-get -y remove --purge opensshd
	;;
2)
	remove_vnc_only
	;;
3)
	...
	;;
4)
	sudo apt-get -y remove --purge teamviewer
	;;
5)
